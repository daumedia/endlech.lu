...
final class RestaurantRepositoryTest extends KernelTestCase
{
    ...
    public function testSortByRatingIsDescending(): void
    {
        // Eindeutige Marker-Ratings ober-/unterhalb aller Fixtures (9.8 … 7.4),
        // damit die relative Reihenfolge deterministisch ist.
        $top = $this->newRestaurant('Top Rating Probe')->setRating(9.99);
        $low = $this->newRestaurant('Low Rating Probe')->setRating(0.5);
        $this->persist($top, $low);

        $items = iterator_to_array($this->repo->findPaginated('rating', 1, 200));
        $ids = array_map(static fn (Restaurant $r) => $r->getId(), $items);

        self::assertLessThan(
            array_search($low->getId(), $ids, true),
            array_search($top->getId(), $ids, true),
        );

        // Gesamte Liste ist monoton fallend (NULL-Ratings ignorieren – sie landen am Ende).
        $previous = null;
        foreach ($items as $restaurant) {
