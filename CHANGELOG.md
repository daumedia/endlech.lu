...
### Fixed
- **Der Asset-Build hängt nicht mehr an der Dokumentation.** Tailwind v4 scannt ohne weitere Angabe das gesamte Projekt – `assets/styles/app.css` schloss bisher nur `public/` aus. Damit landeten auch `docs/` und `features/` im Scan, und Tailwind kann eine als Prosa zitierte Klassenkette nicht von einer verwendeten unterscheiden: `docs/design-system.md` erwähnt unter anderem `ring`, und schon steht die Utility im CSS. Folge war ein CSS-Hash, der sich änderte, sobald jemand Dokumentation schrieb – und ein `verify-assets`, das den Deploy blockte, ohne dass eine Zeile Oberfläche angefasst wurde. Bestand seit dem Merge von `docs/` am 21. August unbemerkt. Mit `@source not` für beide Verzeichnisse erzeugt `npm run build` wieder exakt den committeten `app.b817e2f5.css`; der vorhandene Build bleibt damit gültig und musste nicht neu committet werden.
