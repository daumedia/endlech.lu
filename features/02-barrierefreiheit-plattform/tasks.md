...
Status: `tasked` · Stand: 2026-08-26
...
- [ ] **T01** `[P]` · Limiter `accessibility_report` (sliding_window, 5/Stunde, IP) in
      `config/packages/framework.yaml` anlegen **und** `when@test`-Override auf 10000 —
      Grundlage für T07, `AK-52`
- [ ] **T02** `[P]` · Erklärungs-Parameter in `config/services.yaml`: Konformitätsgrad,
...
- [ ] **T03** `[P]` · Alle neuen Übersetzungsschlüssel für Feature 02 in **allen vier**
...
- [ ] **T04** `[P]` · `AccessibilityReportType`: Beschreibung (Pflicht, `NotBlank`,
...
- [ ] **T05** `[P]` · `AccessibilityReportMailer`: sendet die Meldung als `TemplatedEmail`
...
- [ ] **T06** · `AccessibilityController::index` (GET `/accessibility`, öffentlich, **kein**
...
- [ ] **T07** · `AccessibilityController::report` (POST `/accessibility`): `ActionLimiter`
...
- [ ] **T08** `[P]` · `templates/base.html.twig`: Skip-Link „Zum Inhalt springen" als
...
- [ ] **T09** `[P]` · `assets/styles/app.css`: globaler `:focus-visible`-Fallback (echte
...
- [ ] **T10** `[P]` · `templates/accessibility/` (index + `success.stream.html.twig` +
...
- [ ] **T11** `[P]` · Publikumsseiten `templates/{home,restaurant,about}/` härten: genau
...
- [ ] **T12** `[P]` · Außen-/Datenseiten `templates/{partner,organisation,open}/` härten:
...
- [ ] **T13** `[P]` · `templates/community/vorschlagen` + `suggestion_wizard_controller.ts`:
...
- [ ] **T14** `[P]` · Formularseiten `templates/{security,registration}/` (+ `RegistrationType`):
...
- [ ] **T15** `[P]` · Admin `templates/admin/` + `image_sort_controller.ts` +
...
- [ ] **T16** `[P]` · `templates/profile/index.html.twig`: Abmeldeknopf (CSRF-Formular wie
      im Header), damit Abmelden auf dem Telefon über die Bottom-Nav → Profil erreichbar ist
      — `AK-34`
- [ ] **T17** `[P]` · `templates/email/base.html.twig` und `public/offline.html` härten:
...
- [ ] **T18** `[P]` · Automatischer A11y-Prüflauf: axe-core (WCAG-2.2-AA-Regeln) über eine
...
- [ ] **T19** `[P]` · Strukturelle Functional-Tests (`WebTestCase`): Skip-Link als erstes
...
- [ ] **T20** `[P]` · Konformitäts-Prüfmatrix als Doku-Artefakt (`docs/`): je
...
- [ ] **T21** · Randfälle EC-01–EC-08 durchgehen und absichern: JS aus (Erklärung/Formular
